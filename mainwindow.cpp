...
void MainWindow::onbuttonclicked()
{
    ...
    double k1, k2, k3, k4, propdied, targetwidth, t0;

    k1 = ui->doubleSpinBox->value();
    k2 = ui->doubleSpinBox_2->value();
    k3 = ui->doubleSpinBox_3->value();
    k4 = ui->doubleSpinBox_4->value();
    propdied = ui->doubleSpinBox_5->value();
    targetwidth = ui->doubleSpinBox_6->value();
    t0 = ui->spinBox->value();
...
//    std::ofstream theta1file("theta1hats.txt");
    ...
    for (int sampsizeind=0;sampsizeind<5;sampsizeind++) {
        ...
        for (int final=0;final<mcruns;final++) {
            simdata = generatedata(coxlambda, coxbeta[1], coxbeta[2], coxbeta[3], kkdns[sampsizeind], 1.00);
 ...
 //  		cout << "Sample sizes for local interpolation is: " << n4theta0localsearch[i] << endl;
    ...
    for (int sampsizeind=0;sampsizeind<2;sampsizeind++) {
        ...
        for (int final=0;final<finetunemcruns;final++) {
            simdata = generatedata(coxlambda, coxbeta[1], coxbeta[2], coxbeta[3], n4theta0localsearch[sampsizeind], 1.00);
...
//   	cout << '\a';
    ...
    SampleSizeEstimate = ceil(n4theta0localsearch[0] +
        ...
        (targetwidth-FTfinaltheta0widths[0])/
        (FTfinaltheta0widths[1]-FTfinaltheta0widths[0]));

    SampleSizeEstimate = SampleSizeEstimate/propdied;
...
//    QString k1AsString;
//    ui->label->setNum(k1);
//    k1AsString = QString::number(k1);
//    ui->label->setText(k1AsString);
}
